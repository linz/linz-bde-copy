    ...
    if( $_ =~ /^(\s+)\/\*\s+start_state_machine\s*$/i )
    {
        my $p = $1;
         ...
         while(<>)
         {
             ...
             if( /^\s*state_variable\s+(\S+)\s*$/ ){ $state_variable = $1; next; }
             if( /^\s*input_variable\s+(\S+)\s*$/ ){ $input_variable = $1; next; }
             if( /^\s*action_variable\s+(\S+)\s*$/ ){ $action_variable = $1; next; }
             if( /^\s*default_state\s+(\S+)\s*$/ ){ $default_state = lc($1); next; }
             ...
             {
                 my($name,$proc) = ($1,$2);
                 push(@states,{ id=>$nstate, name=>$name, process=>$proc });
                 $stid{$name} = $nstate;
                 $nstate++;
                 if( $nstate > 255 )
                 {
         ...
         foreach my $s (@states )
         {
             ...
             foreach my $p (split(' ',$s->{process}))
             {
                 ...
                 if( ! $class )
                 { 
                     print ">>> ERROR Invalid class $cname in state $s->{name}\n";
                     $error = 1;
                 }
         ...
         if( ! $error )
         {
             ...
             for my $i (0..15)
             {
                 print $p,"   ",join(",",@cclass[$i*16 .. $i*16+15]);
                 print $i < 15 ? ",\n" : "};\n\n";
             }
             ...
             print $p,"$state_variable = (unsigned char)($prefix\_result & 0xFF);\n";
             print $p,"$action_variable = (unsigned char)($prefix\_result >> 8);\n";
             print "\n";
